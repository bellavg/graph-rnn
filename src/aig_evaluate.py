    ...
    for i in tqdm(range(num_graphs)):
        ...
        for j in range(adj_matrix.shape[0]):
            for k in range(adj_matrix.shape[1]):
                if adj_matrix[j, k] > 0:
                    # Edge type is the value in the matrix
                    G.add_edge(k, j, type=int(adj_matrix[j, k]))

        # Calculate validity
        stats = calculate_aig_structure_validity(G, node_types)
        stats['graph_id'] = i
        ...
        if stats['valid']:
            ...
            # Save graph for metrics calculation
            generated_nx_graphs.append(G)

            # Visualize a subset of valid graphs (e.g., first 10)
            if valid_count <= 10:
                output_file = os.path.join(output_dir, f"valid_aig_{valid_count}.png")
                visualize_aig(adj_matrix, output_file, node_types=node_types)
